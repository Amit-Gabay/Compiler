...
public class AST_STMT_FUNC extends AST_STMT
{
	...
	/*  CONSTRUCTOR(S) */
	/*******************/
	public AST_STMT_FUNC(AST_VAR var, String ID, AST_LIST<AST_EXP> expList, int line)
	{
		this.line = line;
		/******************************/
	...
	public TEMP IRme()
	{
		...
		if (var != null)
		{
			IR.getInstance().Add_IRcommand(new IRcommand_Method_Call(null, class_temp, method_id, tmplist_head));
		}
		else
		{
			...
			{
				IR.getInstance().Add_IRcommand(new IRcommand_Func_Call(null, "_"+ID, tmplist_head));
			}
		}
